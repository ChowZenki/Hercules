	...
	switch( nd->u.scr.shop->currency_type[idx] ) {
		...
		case NSC_INSTANCE_VAR:
			if ( maplist[sd->bl.m].instance_id >= 0 ) {
				k = i64db_iget(instances[maplist[sd->bl.m].instance_id].vars,reference_uid(nd->u.scr.shop->currency[idx],0));
				if( k < qty )
					return 1;
				i64db_iput(instances[maplist[sd->bl.m].instance_id].vars,reference_uid(nd->u.scr.shop->currency[idx],0),k-qty);
				return 0;
			}
			break;
		default:
...
static inline int npc_get_currency_sub(struct map_session_data *sd, struct npc_data *nd, int idx) {
	...
	switch( nd->u.scr.shop->currency_type[idx] ) {
		...
		case NSC_INSTANCE_VAR:
			if ( maplist[sd->bl.m].instance_id >= 0 )
				return i64db_iget(instances[maplist[sd->bl.m].instance_id].vars,reference_uid(nd->u.scr.shop->currency[idx],0));
...
int npc_cashshop_buy2(struct map_session_data *sd, int nameid, int amount, int points) {
	...
	if( (double)shop->item[i].value * amount > INT_MAX ) {
		...
		ShowDebug("(NPC:'%s' (%s,%d,%d), player:'%s' (%d/%d), value:%d, amount:%d)\n",
				  nd->exname, maplist[nd->bl.m].name, nd->bl.x, nd->bl.y, sd->status.name, sd->status.account_id, sd->status.char_id, shop->item[i].value, amount);
		return 5;
	}
